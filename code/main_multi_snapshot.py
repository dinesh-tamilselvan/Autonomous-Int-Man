...
def func(_args):
    ...
    if (not learning_flag): 
        ...
        directory = f"../back_up/version_{int(version)}/arr_{arr_rate_}/train_homo_stream/train_sim_{train_sim}/sim_data/trained_weights/model_weights_itr_{train_iter}"
    ...
    if not data_file.run_coord_on_captured_snap:
        for rep_sim in range(0, max_rep_sim): # through DDPG it will 61 episodes
            ...
            if not data_file.real_time_spawning_flag:  
                ...
                veh_pic = []
                path = '../back_up/version_2/arr_0.1/test_homo_stream/train_sim_1/train_iter_500_copy/pickobj_sim_1'
                ...
                for iter in range(len(sim_obj.unspawned_veh[7])):
                    if sim_obj.unspawned_veh[7][iter].id == 59: pass
                        #print(f'sp_t:{sim_obj.unspawned_veh[5][iter].sp_t}\n pos:{sim_obj.unspawned_veh[5][iter].p_traj},\n vel:{sim_obj.unspawned_veh[5][iter].v_traj},\n acc:{sim_obj.unspawned_veh[5][iter].u_traj}')
                        #print(f'\n time:{sim_obj.unspawned_veh[7][iter].t_ser}')       
                        #print(f'\n\n u_traj:{sim_obj.unspawned_veh[7][iter].u_traj}') 
                        #print(f'\n alpha:{sim_obj.unspawned_veh[7][iter].alpha_dict}') 
                        #rint(f'\n velocity:{sim_obj.unspawned_veh[7][iter].v_traj}') 
                
                
                
                
                #exit()
                # print(f'****************spawned_veh ID:{v.id} \n, pos:{v.p_traj}, \n vel: {v.v_traj},\n u: {v.u_traj} \n time:{v.t_ser}\n{v.sig_stat_t}')
                #exit()
